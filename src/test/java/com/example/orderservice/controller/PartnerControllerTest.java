...
class PartnerControllerTest extends AbstractIntegrationTest {
    ...
    void deletePartner_ShouldReturnNoContent() throws Exception {
        ...
        mockMvc.perform(delete("/partners/" + partner.getId())).andExpect(status().isNoContent());

        mockMvc.perform(get("/partners/" + partner.getId() + "/orders")).andExpect(status().isNotFound());
    }
    ...
    void getOrdersByPartnerId_ShouldReturn404() throws Exception {
        ...
        mockMvc.perform(get("/partners/" + UUID.randomUUID() + "/orders")).andExpect(status().isNotFound());
    }
    ...
    void deletePartner_ShouldReturn404() throws Exception {
        ...
        mockMvc.perform(delete("/partners/" + UUID.randomUUID())).andExpect(status().isNotFound());
    }
    ...
    void getOrdersByPartnerId_nonUuidId_returnsBadRequest() throws Exception {
        ...
        mockMvc.perform(get("/partners/not-a-uuid/orders")).andExpect(status().isBadRequest());
    }
    ...
    void deletePartner_nonUuidId_returnsBadRequest() throws Exception {
        ...
        mockMvc.perform(delete("/partners/not-a-uuid")).andExpect(status().isBadRequest());
    }
}
