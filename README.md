...
> Часть платформы из двух сервисов. Второй репозиторий — **[order-proxy](https://github.com/<org>/order-proxy)**: отказоустойчивый REST-прокси и Telegram-бот поверх этого сервиса.
...
1. Клиент (REST или Telegram) создаёт заказ в `order-proxy`, указывая ссылку на GitHub-репозиторий.
2. `order-proxy` резилиентно проксирует запрос сюда, в `order-service`, где заказ сохраняется в PostgreSQL.
...
5. `order-proxy` потребляет это событие как консьюмер того же топика.
